...
export class OrdersTableDataSource extends DataSource<OrdersTableItem> {
  ...
  constructor(private dataService: DataService, data: OrdersTableItem[]) {
    super();
    this.data = data;
  ...
  private getSortedData(data: OrdersTableItem[]): OrdersTableItem[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this.sort?.active) {
        case 'name': return compare(a.product.productName, b.product.productName, isAsc);
        case 'id': return compare(+a.trackId, +b.trackId, isAsc);
        default: return 0;
      }
    });
  }
